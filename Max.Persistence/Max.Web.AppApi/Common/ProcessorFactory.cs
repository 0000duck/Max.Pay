    ...
    public static class ProcessorUtil
    {
        ...
        static ProcessorUtil()
        {
           ...
           .Where(t => t.IsSubclassOf(typeof(Max.Web.AppApi.Common.BaseRequest)));
            ...
            var members = typeof(Max.Web.AppApi.Common.ApiEnum.BizCode).GetMembers();
            var enumDic = new Dictionary<string, string>();
            foreach (var val in Enum.GetValues(typeof(Max.Web.AppApi.Common.ApiEnum.BizCode)))
            {
                enumDic.Add(Enum.GetName(typeof(Max.Web.AppApi.Common.ApiEnum.BizCode), val), Convert.ToInt32(Enum.Parse(typeof(Max.Web.AppApi.Common.ApiEnum.BizCode), val.ToString())).ToString());
            }
