...
namespace Max.Web.Presentation.Common
{
    public static class ParametersHelper
    {
        ...
        /// <param name="needEncode">是否需要编码</param>
        /// <returns></returns>
        public static string EncryptUserIdByDes(string userId, bool needEncode=false)
        {
            ...
            var userIdEnc = CryptographyUtils.DesEncrypt(val, UserIdDesSecretKey);
            if (needEncode)
            {
                userIdEnc = HttpUtility.UrlEncode(userIdEnc);
            }
        ...
        /// <param name="needDecode">是否需要URL解码</param>
        /// <returns></returns>
        public static string DecryptUserIdByDes(string encryptUserId, bool needDecode = false)
        {
            ...
            if (needDecode)
            {
                encryptUserId = HttpUtility.UrlDecode(encryptUserId);
            }
        ...
        public static string FormatIdCardNo(string idCardNo)
        {
            ...
            return idCardNo.Substring(0, 2).PadRight(len - 2, '*') + idCardNo.Substring(len - 2);
        }
