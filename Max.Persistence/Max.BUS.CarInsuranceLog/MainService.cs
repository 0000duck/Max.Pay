...
namespace Max.BUS.CarInsuranceLog
{
    public class MainService
    {
        ...
        public bool Start()
        {
            try
            {
                this.bus.Subscribe<ICarInsuranceMessage>("", msg =>
                {
                    log.Info(msg.ToJson());
                    //var dbName = "CarInsuranceLog" + DateTime.Now.ToString("yyyyMM");
        ...
        private string GetDbName(int type)
        {
            ...
            string dc = "CarInsuranceLog" + DateTime.Now.ToString("yyyyMM");
            switch (type)
            {
                case 1:
                    dbName = dc + "_HuaAn";
